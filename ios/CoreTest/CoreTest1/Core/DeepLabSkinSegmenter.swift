...
////  NevoScan
////
////  DeepLabV3: вход — тензор «image» float32, RGB, NCHW [1,3,256,256], значения 0…255.
////  Выход — «mask», сигмоида.
////
//
//import CoreGraphics
...
////    }
////    
////    /// Возвращает бинарную маску [1,1,256,256], значения 0/1 (float32), как в Python `(sigmoid > 0.5)`.
...
////    }
////    
////    /// Маска 256×256 и раскладка letterbox (для приведения маски к размеру исходного кропа).
...
////    }
////    
////    /// Ч/б маска в размере входного изображения (как `cv2.resize(mask, (W, H))` после letterbox).
...
////    }
////    
////    /// Маска 256×256, приведённая к размеру кропа по той же геометрии letterbox.
...
//// MARK: - Letterbox 256
//
///// Геометрия letterbox: обратное отображение маски 256×256 → исходный размер кропа.
...
//    }
//
//    /// Оставляет только RGB для модели: при наличии альфы кадр кладётся на белый фон (корректнее, чем просто отбросить 4-й байт у premultiplied RGBA).
...
//
//    /// Как в Python: `torch.sigmoid(x)` с последующим порогом `> 0.5` (значение из Core ML может быть до/после сигмоиды в графе — в коде порог всегда после sigmoid).
...
//    }
//
//    /// Маска 256×256 → вырез области без паддинга → `cv2.resize` к размеру исходного кропа (nearest).
...
//  Порог маски: `> 0.5` по вероятности (sigmoid уже в графе).
//
...
/// PIL / `Image.fromarray` обычно в sRGB; `DeviceRGB` на iOS может давать другой ответ модели.
...
// MARK: - Resize 256 (как `transforms.Resize` в main.py)

/// Геометрия 256×256 → исходный кроп: `contentRect256` — область маски в квадрате 256 (при stretch — весь кадр); обратное масштабирование как `cv2.resize(..., INTER_NEAREST)`.
struct LetterboxLayout: Sendable {
    let originalWidth: Int
    let originalHeight: Int
    /// Область содержимого кропа в координатах 256×256
...
class Segmenter {
    ...
    func runSegmentation(on image: UIImage) async -> (maskResult: UIImage?, maskArray: MLMultiArray?) {
        ...
        do {
            let prepared = uiImagePreparedForDeepLabInference(image)
            guard let cgImage = prepared.cgImage else {
                print("Segmenter: после подготовки cgImage == nil (часто из-за UIImage(ciImage:) без растра).")
                return (nil, nil)
            }
            let letter = try resizeStretch256(prepared: prepared, cgImage: cgImage)
            guard let letterCg = letter.image.cgImage else {
    ...
    private func convertMultiArrayToMask(_ array: MLMultiArray) -> UIImage {
        ...
        if groups == 1 {
            // Выход `seg_to_core.py`: уже sigmoid 0…1; как `preds = torch.sigmoid(output) > 0.5`
            for y in 0..<h {
                for x in 0..<w {
                    let v = mlaReadFloat(array, linear: mlaLinearOffset(strides: strides, indices: indicesYX(y, x)))
                    let p = min(max(v, 0), 1)
    ...
    /// Как `transforms.Resize((256, 256))` в `main.py`: ровно 256×256, без сохранения пропорций; sRGB; см. комментарий в шапке про `interpolationQuality`.
    private func resizeStretch256(prepared: UIImage, cgImage: CGImage) throws -> Letterbox256 {
        ...
        // PIL / torchvision `Resize`: bilinear; у CG `.high` даёт более «тяжёлое» сглаживание, чем PIL.
    ...
    private func rgbNCHWFloatTensor(from256 cgImage: CGImage) throws -> MLMultiArray {
        ...
        // Уже 256×256 после resizeStretch256 — копия 1:1 без повторной фильтрации (иначе дважды «мягкий» resize).
        ...
        for y in 0..<h {
            for x in 0..<w {
                // После translate+scale строка 0 буфера = верх кадра; индекс [*, y, x] как у PyTorch (y сверху вниз).
    ...
    /// Маска 256×256 → вырез `contentRect256` → масштаб к кропу как `cv2.resize` по умолчанию (`INTER_LINEAR` в main.py).
    func binaryMaskUIImage(fromBinaryMask256 mask: MLMultiArray, letterboxLayout: LetterboxLayout) throws -> UIImage {
        let mask256 = try binaryMaskUIImage(fromBinaryMask256: mask)
        guard let cg256 = mask256.cgImage else {
            throw DeepLabSegmentationError.couldNotPrepareImage

...
internal fun copyAssetToFilesDir(context: Context, assetName: String): File {
    ...
    if (!assetNames.contains(assetName)) {
        // Если модель удалили из assets, удаляем stale-копию во внутреннем хранилище.
    ...
    // Всегда перезаписываем файл, чтобы не использовать устаревшую кэш-копию модели.
...
internal fun binaryMaskNearestToFloatN11hw(
    src: ByteArray,
    sw: Int,
    sh: Int,
    dw: Int,
    ...
    for (y in 0 until dh) {
        for (x in 0 until dw) {
            ...
            val v = src[sy * sw + sx]
            val f = if (v != 0.toByte()) 1f else 0f
            out[y * dw + x] = if (f > 0.5f) 1f else 0f
        }
    }
    return out
}
...
internal fun binaryMaskNearestExpandedToNchw(
    ...
    val base = binaryMaskNearestToFloatN11hw(src, sw, sh, dw, dh)
